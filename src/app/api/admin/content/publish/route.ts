...
import { scrubCopy } from "@/lib/voice";
...
/* Per-network variant. Generated by /api/admin/content/generate for each
 * platform, or hand-authored by the operator in the drawer. */
interface NetworkVariant {
  hook?: string;
  copy: string;
  hashtags?: string[]; // Instagram-only; ignored for LI / FB.
}

interface SocialPostPayload {
  // New shape (preferred): per-network variants.
  linkedin?: NetworkVariant;
  instagram?: NetworkVariant;
  facebook?: NetworkVariant;
  // Legacy shape — kept for backwards compat with drafts authored before
  // the per-network refactor. If present and the new keys are missing,
  // the publisher fans the same `copy`/`hook` to all 3 channels.
  network?: "linkedin" | "instagram" | "facebook" | "all";
  copy?: string;
  hook?: string;
  // Shared (apply to all 3 channels).
  cta_text?: string;
  cta_url?: string;
  image_url?: string;
  image_prompt?: string;
...
/* Return the per-channel variant. Falls back to the legacy top-level
 * {copy, hook} for drafts authored before the per-network refactor. */
function variantFor(payload: SocialPostPayload, channel: ChannelKey): NetworkVariant | null {
  const fresh = payload[channel];
  if (fresh && typeof fresh.copy === "string" && fresh.copy.length > 0) {
    return fresh;
  }
  // Legacy fallback — use the flat fields.
  if (payload.copy && typeof payload.copy === "string") {
    return { hook: payload.hook, copy: payload.copy };
  }
  return null;
}

function composeChannelText(payload: SocialPostPayload, channel: ChannelKey): string | null {
  const v = variantFor(payload, channel);
  if (!v) return null;
  const parts: string[] = [];
  if (v.hook) parts.push(scrubCopy(v.hook));
  if (v.copy && v.copy !== v.hook) parts.push(scrubCopy(v.copy));
  if (channel === "instagram" && Array.isArray(v.hashtags) && v.hashtags.length > 0) {
    // Append hashtags as a final line. Normalize: strip leading # if the
    // generator already added one, then re-add to guarantee uniformity.
  ...
  if (payload.cta_text && payload.cta_url) {
    const url = payload.cta_url.startsWith("/")
      ...
      : payload.cta_url;
    parts.push(`${scrubCopy(payload.cta_text)} → ${url}`);
  }
  return parts.filter(Boolean).join("\n\n");
}

function altTextFor(payload: SocialPostPayload, channel: ChannelKey): string {
  const v = variantFor(payload, channel);
  const base = scrubCopy((v?.hook || v?.copy || "ARTO post").trim());
...
> {
  ...
  /* Decide channels. Precedence:
   *  1. NEW shape — publish to every channel that has a per-network variant.
   *  2. LEGACY shape — use the top-level `network` field + legacy {copy, hook}
   *     applied uniformly to the selected channels.
   *  3. Neither — error. */
  const channelKeys: ChannelKey[] = [];
  const hasNewShape = !!(p.linkedin?.copy || p.instagram?.copy || p.facebook?.copy);
  if (hasNewShape) {
    if (p.linkedin?.copy) channelKeys.push("linkedin");
    if (p.instagram?.copy) channelKeys.push("instagram");
    if (p.facebook?.copy) channelKeys.push("facebook");
  } else {
    if (!p.copy || typeof p.copy !== "string") {
      return { ok: false, error: "missing payload (need per-network variants or legacy copy)" };
    }
  ...
  if (channelKeys.length === 0) {
    return { ok: false, error: "no channels resolved from payload" };
  }
  ...
  for (const key of channelKeys) {
    const text = composeChannelText(p, key);
    if (!text) {
      partialErrors.push(`${key}: missing copy for this channel`);
      continue;
    }
    const altText = altTextFor(p, key);
